...
/// <summary>
/// Fused step kernel for double pole balancing (no velocity variant).
/// One thread per world. Each launch processes TicksPerLaunch ticks.
/// Each tick: observe -> NN forward pass -> action -> 2x RK4 physics -> terminal check.
/// Physics matches Colonel.Core.Playground.DoublePoleCart exactly.
/// </summary>
public static class DoublePoleStepKernel
{
        ...
        ArrayView<float> state)
    {
        ...
        float s0 = state[stateBase];     // cart position
        float s1 = state[stateBase + 1]; // cart velocity
        float s2 = state[stateBase + 2]; // pole1 angle
        float s3 = state[stateBase + 3]; // pole1 angular velocity
        float s4 = state[stateBase + 4]; // pole2 angle
        float s5 = state[stateBase + 5]; // pole2 angular velocity
        ...
        for (int tick = 0; tick < config.TicksPerLaunch; tick++)
        {
            ...
            // === 2. NN forward pass ===
            InlineNN.ForwardPass(nn, observations, actions, worldIdx,
                config.TotalNodes, config.TotalEdges, config.NumRows,
            ...
            // === 4. Physics: 2x RK4 (matching Colonel's Tick) ===
            PerformRK4(action, ref s0, ref s1, ref s2, ref s3, ref s4, ref s5);
            PerformRK4(action, ref s0, ref s1, ref s2, ref s3, ref s4, ref s5);

            steps++;

            // === 5. Jiggle tracking (Gruau's anti-oscillation metric) ===
            ...
            if (outOfBounds || steps >= config.MaxSteps)
            {
                ...
                // Gruau bonus: reward smooth control for survivors of full episode
    ...
    /// <summary>
    /// One RK4 integration step advancing state by TimeDelta.
    /// Matches Colonel's Rk4() method exactly.
    /// </summary>
        ...
        ref float s0, ref float s1, ref float s2, ref float s3, ref float s4, ref float s5)
    {
        ...
        // Final combination: y + h/6 * (k1 + 2*(k2+k3) + k4)
    ...
    /// <summary>
    /// Computes cart and pole accelerations.
    /// Matches Colonel's DoublePoleCart.Step() exactly.
    /// </summary>
