...
/// <summary>
/// GPU kernels for rigid body joint constraint solver.
/// Ports the CPU implementation (RigidBodyJointSolver) to ILGPU.
/// Based on Box2D's sequential impulse joint solver.
/// </summary>
public static class GPURigidBodyJointKernels
{
    ...
    #region Initialize Joint Constraints Kernel
    ...
    /// </summary>
    public static void InitializeJointConstraintsKernel(
        Index1D jointIndex,
        ...
        float dt)
    {
        ...
        var bodyA = bodies[joint.BodyA];
        var bodyB = bodies[joint.BodyB];

        // Skip if both bodies are static
        if (bodyA.InvMass == 0f && bodyB.InvMass == 0f)
        {
            // Initialize to zero constraint (no-op)
            constraints[jointIndex] = new GPUJointConstraint
            {
        ...
        constraints[jointIndex] = constraint;
    }

    #endregion

    #region Solve Joint Velocities Kernel
    ...
    /// One thread per joint.
    /// </summary>
    public static void SolveJointVelocitiesKernel(
        Index1D jointIndex,
        ArrayView<GPURigidBody> bodies,
        ArrayView<GPUJointConstraint> constraints,
        float dt)
    {
        var constraint = constraints[jointIndex];
        ...
        if (constraint.EnableMotor != 0 && constraint.MotorMass > 0f)
        {
            ...
            Atomic.Add(ref bodies[constraint.BodyAIndex].AngularVel, -bodyA.InvInertia * motorImpulse);
            Atomic.Add(ref bodies[constraint.BodyBIndex].AngularVel, bodyB.InvInertia * motorImpulse);
        ...
        if (constraint.EnableLimits != 0 && constraint.AngleLimitMass > 0f)
        {
            ...
            if (XMath.Abs(limitImpulse) > Epsilon)
            {
                ...
                Atomic.Add(ref bodies[constraint.BodyAIndex].AngularVel, -bodyA.InvInertia * limitImpulse);
                Atomic.Add(ref bodies[constraint.BodyBIndex].AngularVel, bodyB.InvInertia * limitImpulse);
        ...
        Atomic.Add(ref bodies[constraint.BodyAIndex].VelX, -bodyA.InvMass * lambdaX);
        Atomic.Add(ref bodies[constraint.BodyAIndex].VelY, -bodyA.InvMass * lambdaY);
        Atomic.Add(ref bodies[constraint.BodyAIndex].AngularVel, -bodyA.InvInertia * (rAX * lambdaY - rAY * lambdaX));

        // Body B: add impulse
        Atomic.Add(ref bodies[constraint.BodyBIndex].VelX, bodyB.InvMass * lambdaX);
        Atomic.Add(ref bodies[constraint.BodyBIndex].VelY, bodyB.InvMass * lambdaY);
        Atomic.Add(ref bodies[constraint.BodyBIndex].AngularVel, bodyB.InvInertia * (rBX * lambdaY - rBY * lambdaX));
    ...
    #region Solve Joint Positions Kernel
    ...
    /// One thread per joint.
    /// </summary>
    public static void SolveJointPositionsKernel(
        Index1D jointIndex,
        ArrayView<GPURigidBody> bodies,
        ArrayView<GPUJointConstraint> constraints)
    {
        ...
        if (errorLength > LinearSlop)
        {
            ...
            Atomic.Add(ref bodies[constraint.BodyAIndex].X, -bodyA.InvMass * impulseX);
            Atomic.Add(ref bodies[constraint.BodyAIndex].Y, -bodyA.InvMass * impulseY);
            Atomic.Add(ref bodies[constraint.BodyAIndex].Angle, -bodyA.InvInertia * (rAX * impulseY - rAY * impulseX));

            // Body B: add correction
            Atomic.Add(ref bodies[constraint.BodyBIndex].X, bodyB.InvMass * impulseX);
            Atomic.Add(ref bodies[constraint.BodyBIndex].Y, bodyB.InvMass * impulseY);
        ...
        if (constraint.EnableLimits != 0 && constraint.AngleLimitMass > 0f)
        {
            ...
            float angleB = bodies[constraint.BodyBIndex].Angle;
            ...
            if (XMath.Abs(angleError) > Epsilon)
            {
                ...
                Atomic.Add(ref bodies[constraint.BodyAIndex].Angle, -bodyA.InvInertia * angleImpulse);
                Atomic.Add(ref bodies[constraint.BodyBIndex].Angle, bodyB.InvInertia * angleImpulse);
            }
        }
    }
